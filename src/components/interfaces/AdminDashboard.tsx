...
export const AdminDashboard = ({ onBack }: { onBack: () => void }) => {
  ...
  const { menuItems, categories, tables } = useMenu();
...
const pageCount = (array: any[]) => Math.ceil(array.length / itemsPerPage);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      toast({ title: "Error", description: "Failed to load orders for analytics", variant: "destructive" });
    });

    return () => unsub();
  }, [toast]);

  // ---------- Filter orders by pastDay ----------
  useEffect(() => {
    const selectedDate = new Date(pastDay);
    const filtered = orders.filter(o => {
      const d = o.created_at?.toDate ? o.created_at.toDate() : (o.created_at instanceof Date ? o.created_at : new Date(o.created_at));
    ...
    <div className="min-h-screen bg-gradient-subtle">
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <Card><CardContent className="p-6"><div className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground">Total Orders</p><p className="text-2xl font-bold">{orders.length}</p></div><BarChart3 className="w-8 h-8 text-primary"/></div></CardContent></Card>
          ...
          <Card><CardContent className="p-6"><div className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground">Active Orders</p><p className="text-2xl font-bold">{(ordersByStatus.pending || 0) + (ordersByStatus.accepted || 0) + (ordersByStatus.preparing || 0) + (ordersByStatus.ready || 0)}</p></div><ChefHat className="w-8 h-8 text-warning"/></div></CardContent></Card>
          <Card><CardContent className="p-6"><div className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground">Completed Today</p><p className="text-2xl font-bold">{orders.filter(o => {
            const d = o.created_at?.toDate ? o.created_at.toDate() : (o.created_at instanceof Date ? o.created_at : new Date(o.created_at));
            const today = new Date(); return d.getFullYear() === today.getFullYear() && d.getMonth() === today.getMonth() && d.getDate() === today.getDate() && o.status === "completed";
          }).length}</p></div><TrendingUp className="w-8 h-8 text-primary"/></div></CardContent></Card>
        </div>
        ...
        <Tabs defaultValue="items" className="space-y-6">
          ...
          {/* ... Items, Categories, Tables, Orders, Analytics, Status tabs ... */}
  ...
  itemsPerPage={itemsPerPage}
                  ...
                  {orders.slice(0, 10).map((o) => (
                    <div key={o.id} className="flex justify-between items-center border rounded p-3">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">{(o.created_at?.toDate ? o.created_at.toDate() : (o.created_at instanceof Date ? o.created_at : new Date(o.created_at))).toLocaleString()}</p>
          ...
          <TabsContent value="analytics">
            <Card>
              ...
              <CardContent>
                {/* example simple ranking computed from orders */}
                {(() => {
